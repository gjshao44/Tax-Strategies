...
""",
        ...
        "col_bond": "INPUT: Taxable Bond Int",
        ...
        "lab_roth_optimum": "Optimal Roth conversion, holding your current SS/muni/capital-gains/withdrawal settings fixed:",
        ...
        "muni_bracket_label": "Your Effective Marginal Rate",
        "muni_yield_label": "Muni Yield (%)",
        "muni_tey_result": "Tax-Equivalent Yield",
        "muni_alloc_h": "Optimal Muni Allocation",
        "muni_alloc_desc": "Compares net worth outcomes across different muni allocation levels within your bond sleeve (taxable bond interest vs. muni interest). A higher muni % trades raw yield for tax savings and MAGI reduction. Equity dividend income is unaffected -- it isn't fungible with bonds.",
        ...
        "tab_state": "🏛️ Fed & State Tax",
        "state_h": "Federal + State Tax by Residency",
        "state_desc": "A **simplified planning estimate**, not tax advice. Compares your Year-1 retirement income (from the sidebar) against 10 states' actual rules -- brackets/flat rate, standard deduction or personal exemption, Social Security taxability, how IRA/401(k)/pension withdrawals are treated (exempt/ordinary/partial exclusion), age-65+ deductions/exemptions, and property tax on a home value you enter. Every figure is sourced -- see the state facts panel below. Income-tested senior relief programs (circuit breakers, freezes, deferrals, rebates) are described but **not** auto-applied, since eligibility depends on income limits this app doesn't model -- check those manually if your income is near the stated limits. **Itemized deductions (mortgage interest, charitable giving, medical expenses) are not modeled** -- only the standard deduction/personal exemption path is used for every state. PA, IL, and NJ don't offer state itemizing at all, so this doesn't affect them; CA, OR, and NC do allow itemizing and your actual state tax could be lower than shown here if itemizing beats your standard deduction.",
        "state_fed_h": "Federal Tax Context (Year 1)",
        "state_fed_desc": "Reusing the same Year-1 federal calculation as the rest of this app (standard deduction + age-65 addition + OBBBA temporary senior bonus deduction, the Social Security taxability worksheet, and NIIT).",
        "state_fed_magi": "Year 1 MAGI",
        "state_fed_tax": "Year 1 Federal Tax",
        "state_fed_obbba": "OBBBA Senior Bonus Deduction",
        "state_fed_obbba_help": "Temporary 2025-2028 deduction (sunsets after 2028), phased out above \\$75k (single) / \\$150k (MFJ) MAGI.",
        "state_input_h": "Your State & Home",
        "state_select_label": "State of Residency",
        "state_home_value_label": "Home Value ($, for property tax estimate)",
        "state_property_basis_label": "Property Tax Basis",
        "state_property_basis_avg": "Statewide average (existing owners)",
        "state_property_basis_new": "New buyer / recent purchase (where available)",
        "state_property_basis_na_note": "No separate new-buyer rate available for this state -- using the statewide average.",
        "state_compare_h": "Estimated State Tax by State (Year 1)",
        "state_compare_desc": "Your selected state highlighted in red. Bars combine estimated state income tax + property tax on the home value entered above.",
        "state_col_state": "State",
        "state_col_income_tax": "State Income Tax",
        "state_col_property_tax": "Property Tax",
        "state_col_total": "Total State + Property",
        "state_col_combined": "Combined Fed + State + Property",
        "state_breakdown_h": "Breakdown for {state}",
        "state_excluded_label": "Retirement Income Excluded from State Tax",
        "state_taxable_base_label": "State Taxable Income",
        "state_deduction_label": "deduction/exemption applied",
        "state_senior_deduction_label": "incl. age-65+ addition",
        "state_fed_subtraction_label": "Federal Tax Liability Subtracted (OR only)",
        "state_deepdive_h": "📋 State Facts & Senior Benefits — {state}",
        "state_ss_label": "Social Security Taxable?",
        "state_retirement_label": "IRA/401(k)/Pension Withdrawals Taxable?",
        "state_property_rate_label": "Effective Property Tax Rate",
        "state_senior_credit_label": "Senior Income Tax Credits/Deductions",
        "state_property_relief_label": "Senior Property Tax Relief Programs",
        "state_no_income_tax": "No state income tax",
        "state_ss_yes": "Yes",
        "state_ss_no": "No",
        "state_ri_exempt": "Fully exempt",
        "state_ri_ordinary": "Fully taxable (ordinary income)",
        "state_ri_partial": "Partially exempt (income-tested)",
    },
    "Chinese": {
        "title": "🛡️ 退休税务策略规划器",
...
""",
        ...
        "lab_roth_optimum": "最优 Roth 转换金额（其他设置——社保/市政债/资本利得/提款策略——保持当前值不变）：",
        ...
        "muni_tey_h": "税等价收益率分析",
        "muni_bracket_label": "您的有效边际税率",
        "muni_yield_label": "市政债券收益率 (%)",
        "muni_tey_result": "税等价收益率",
        "muni_alloc_h": "最优市政债券配置",
        "muni_alloc_desc": "比较您固定收益(债券)部分中，应税债券利息与市政债券利息不同配置比例下的净资产结果。更高的市政债券比例以原始收益换取税收节省和MAGI降低。股票股息收入不受影响——它与债券不可互换。",
        ...
        "cg_ref_h": "资本利得税率参考",
        "tab_state": "🏛️ 联邦与州税",
        "state_h": "按居住州划分的联邦+州税",
        "state_desc": "**简化规划估算，非税务建议**。将您第1年的退休收入（来自侧边栏）代入10个州的实际规则进行比较——累进税率/统一税率、标准扣除额或个人免税额、社保是否应税、IRA/401(k)/养老金提款的处理方式（免税/普通应税/部分免除）、65岁以上的扣除额/免税额，以及您输入的房屋价值对应的房产税。每个数字均标明来源——见下方州级信息面板。与收入挂钩的老年人税务减免项目（断路器抵免、评估冻结、递延、返还）仅作说明，**不会**自动计入计算，因为其资格取决于本工具未建模的收入限制——如果您的收入接近这些限制，请自行核实。**未建模分项扣除**（房贷利息、慈善捐赠、医疗支出）——所有州均仅按标准扣除额/个人免税额计算。PA、IL、NJ本身不提供州级分项扣除，故不受影响；CA、OR、NC允许分项扣除，如果分项扣除对您更有利，实际州税可能低于此处显示的数值。",
        "state_fed_h": "联邦税背景（第1年）",
        "state_fed_desc": "复用本工具其他标签页相同的第1年联邦税计算（标准扣除额+65岁附加额+OBBBA临时老年人额外扣除、社保应税额工作表，以及NIIT）。",
        "state_fed_magi": "第1年 MAGI",
        "state_fed_tax": "第1年联邦税",
        "state_fed_obbba": "OBBBA老年人额外扣除",
        "state_fed_obbba_help": "2025-2028年临时扣除（2028年后到期），MAGI超过\\$75k（单身）/\\$150k（夫妻联合）后逐步减少。",
        "state_input_h": "您的州与房产",
        "state_select_label": "居住州",
        "state_home_value_label": "房屋价值（$，用于估算房产税）",
        "state_property_basis_label": "房产税计算口径",
        "state_property_basis_avg": "全州平均（现有房主）",
        "state_property_basis_new": "新购房者/近期购房（如有该数据）",
        "state_property_basis_na_note": "该州暂无单独的新购房者税率数据 —— 使用全州平均税率。",
        "state_compare_h": "各州估算州税对比（第1年）",
        "state_compare_desc": "红色高亮为您选择的州。柱状图为估算州所得税+上方输入房屋价值对应的房产税之和。",
        "state_col_state": "州",
        "state_col_income_tax": "州所得税",
        "state_col_property_tax": "房产税",
        "state_col_total": "州税+房产税合计",
        "state_col_combined": "联邦+州+房产税合计",
        "state_breakdown_h": "{state} 明细",
        "state_excluded_label": "州税中免除的退休收入",
        "state_taxable_base_label": "州应税收入",
        "state_deduction_label": "已应用扣除额/免税额",
        "state_senior_deduction_label": "含65岁以上附加额",
        "state_fed_subtraction_label": "已扣除的联邦税额（仅俄勒冈州）",
        "state_deepdive_h": "📋 州信息与老年人福利 — {state}",
        "state_ss_label": "社保是否应税？",
        "state_retirement_label": "IRA/401(k)/养老金提款是否应税？",
        "state_property_rate_label": "实际房产税率",
        "state_senior_credit_label": "老年人所得税抵免/扣除",
        "state_property_relief_label": "老年人房产税减免项目",
        "state_no_income_tax": "无州所得税",
        "state_ss_yes": "是",
        "state_ss_no": "否",
        "state_ri_exempt": "完全免税",
        "state_ri_ordinary": "完全应税（普通所得）",
        "state_ri_partial": "部分免税（与收入挂钩）",
    }
}
