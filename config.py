...
""",
        ...
        "state_property_rate_label": "Avg. Effective Property Tax Rate",
...
""",
        ...
        "state_desc": "**简化规划估算，非税务建议**。将您第1年的退休收入（来自侧边栏）代入10个州的实际规则进行比较——累进税率/统一税率、标准扣除额或个人免税额、社保是否应税、IRA/401(k)/养老金提款的处理方式（免税/普通应税/部分免除）、65岁以上的扣除额/免税额，以及您输入的房屋价值对应的房产税。每个数字均标明来源——见下方州级信息面板。与收入挂钩的老年人税务减免项目（断路器抵免、评估冻结、递延、返还）仅作说明，**不会**自动计入计算，因为其资格取决于本工具未建模的收入限制——如果您的收入接近这些限制，请自行核实。",
        ...
        "state_property_rate_label": "平均实际房产税率",
